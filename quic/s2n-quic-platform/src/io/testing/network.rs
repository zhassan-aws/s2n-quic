...
impl Default for Buffers {
    fn default() -> Self {
        Self {
            ...
            next_port: Arc::new(AtomicU16::new(4433)),
        }
    }
}
...
impl Buffers {
    pub fn tx<F: FnOnce(&mut Queue)>(&self, handle: SocketAddress, f: F) {
        ...
        f(lock.tx.get_mut(&handle).unwrap())
    }

    pub fn rx<F: FnOnce(&mut Queue)>(&self, handle: SocketAddress, f: F) {
        let mut lock = self.inner.lock().unwrap();
        f(lock.rx.get_mut(&handle).unwrap())
    }

    pub fn pending_transmissions<F: FnMut(Packet)>(&self, max_transmissions: u64, mut f: F) {
        let mut lock = self.inner.lock().unwrap();
